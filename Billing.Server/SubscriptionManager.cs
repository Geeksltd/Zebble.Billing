namespace Zebble.Billing
{
	...
	class SubscriptionManager : ISubscriptionManager
	{
		...
		public async Task<Subscription> GetSubscriptionStatus(string userId)
		{
			...
			var subscription = FindValidSubscription(subscriptions);
		...
		static Subscription FindValidSubscription(Subscription[] subscriptions)
		{
			subscriptions = subscriptions.Where(x => x.IsStarted()).ToArray();

			// Prioritize non-expirables
			var subscription = subscriptions.LastOrDefault(x => x.ExpirationDate == null && !x.IsCanceled());
			if (subscription is not null) return subscription;

			// Then search in expirables
			subscriptions = subscriptions.Where(x => x.ExpirationDate.HasValue).OrderBy(x => x.ExpirationDate).ToArray();

			subscription = subscriptions.LastOrDefault(x => x.CancellationDate == null);
			if (subscription is not null) return subscription;

			return subscriptions.OrderBy(x => x.CancellationDate).LastOrDefault();
		}
